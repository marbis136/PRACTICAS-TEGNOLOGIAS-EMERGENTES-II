...
X = iris.data[:, :2]  # Usamos solo las dos primeras características para graficar
y = iris.target
...
# Crear el modelo SVM de margen suave
svm_model = SVC(kernel='linear', C=0.5, decision_function_shape='ovo')  # 'ovo' para multiclase
svm_model.fit(X_train, y_train)

# Predecir en el conjunto de prueba
y_pred = svm_model.predict(X_test)
accuracy = accuracy_score(y_test, y_pred)
print(f"Precisión del modelo SVM (Margen Suave): {accuracy:.2f}")
...
def plot_decision_boundaries(X, y, model, class_names):
    ...
    plt.title('Fronteras de decisión con SVM (Margen Suave)')
    plt.grid(True)
    plt.show()
